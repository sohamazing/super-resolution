...
class SuperResDataset(Dataset):
    """A custom dataset that returns both HR and LR images."""
...
def sample_and_log_images(model, scheduler, loader, epoch):
    ...
    lr_upscaled = nn.functional.interpolate(lr_sample, scale_factor=4, mode='bicubic', align_corners=False)
...
def main(args):
    ...
    if args.resume_epoch > 0:
        ...
        model.load_state_dict(torch.load(config.CHECKPOINT_DIR / f"diffusion_model_epoch_{start_epoch}.pth"))
        try:
            optimizer.load_state_dict(torch.load(config.CHECKPOINT_DIR / f"optimizer_diffusion_epoch_{start_epoch}.pth"))
    ...
    lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=config.NUM_EPOCHS, eta_min=1e-7, last_epoch=start_epoch-1)

    print("--- Starting Diffusion Model Training ---")
    for epoch in range(start_epoch, config.NUM_EPOCHS):
        loop = tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{config.NUM_EPOCHS}")
        avg_loss = 0.0
        ...
        for i, (hr_batch, lr_batch) in enumerate(loop):
            ...
            lr_upscaled = nn.functional.interpolate(lr_batch, scale_factor=4, mode='bicubic', align_corners=False)
        ...
        if (epoch + 1) % 25 == 0:
            ...
            torch.save(model.state_dict(), config.CHECKPOINT_DIR / f"diffusion_model_epoch_{epoch+1}.pth")
            torch.save(optimizer.state_dict(), config.CHECKPOINT_DIR / f"optimizer_diffusion_epoch_{epoch+1}.pth")
